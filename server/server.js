...
const serviceAccountKey = JSON.parse(
  fs.readFileSync(new URL('./blog-website-react-9778f-firebase-adminsdk-fbsvc-b7637b2b32.json', import.meta.url))
);
...
server.post("/notifications", verifyJWT, (req, res) => {
    ...
    console.log("Received request:", { user_id, page, filter, deletedDocCount });
...
// Route to check for new notifications
server.get("/new-notification", verifyJWT, async (req, res) => {
    try {
      let user_id = req.user;
  
      // Check if there are any unseen notifications for the user
  ...
  server.post("/delete-notification", verifyJWT, async (req, res) => {
    try {
        ...
        console.log(`Notification ${notification_id} deleted by user ${user_id} at ${new Date().toISOString()}`);
...
server.post("/add-reply", verifyJWT, async (req, res) => {
    try {
        const { reply, blog_id, blog_author, comment_id, notification_id } = req.body;
        const user_id = req.user;

        if (!reply || !blog_id || !comment_id) {
            return res.status(400).json({ error: "Missing required fields" });
        }

        // Parse reply data if it's a JSON string
        let replyData = reply;
        let replyText = reply;
        
        try {
            // If reply is a JSON string containing additional info
            const parsedReply = JSON.parse(reply);
            replyText = parsedReply.text;
            replyData = {
                text: parsedReply.text,
                username: parsedReply.username || "Dishang18",
                timestamp: parsedReply.timestamp || "2025-04-27 09:59:56"
            };
        } catch (e) {
        ...
        const newReply = new Reply({
            ...
            comment_id,
            metadata: {
                username: replyData.username || "Dishang18",
                timestamp: replyData.timestamp || "2025-04-27 09:59:56"
            }
        });
        ...
        // Update the comment to include this reply
        await Comment.findByIdAndUpdate(
            comment_id,
            { $push: { replies: savedReply._id } }
        );

        // Create a notification for the comment author (if it's not the user)
        const comment = await Comment.findById(comment_id);
        
        if (comment && comment.user_id.toString() !== user_id) {
            // Create notification
            const newNotification = new Notification({
                type: "reply",
                blog: {
                    blog_id,
                    title: (await Blog.findById(blog_id))?.title || "Blog Post"
                },
                reply: {
                    reply: replyText,
                    metadata: {
                ...
                user: user_id,
                notification_for: comment.user_id,
                comment: {
                    _id: comment_id
                }
            });
...
server.post("/read-all-notifications", verifyJWT, async (req, res) => {
    try {
        const user_id = req.user;
        ...
        // Reset the new notification flag in user model if you have one
        await User.findByIdAndUpdate(
            user_id,
            { $set: { new_notification_available: false } }
        );
...
server.post("/user-written-blogs", verifyJWT, async(req, res) => {
    try {
        ...
        query = query || "";
        deletedDocCount = parseInt(deletedDocCount) || 0;
        ...
        // Fix the select statement - remove space before -_id
        ...
        .limit(maxLimit)
        .sort({ publishAt: -1 })
        .select("title banner publishAt blog_id activity des draft -_id"); // FIX HERE: removed space before -_id
        
        return res.status(200).json({ blogs });
    } catch (err) {
        console.error("Error in /user-written-blogs:", err);
        return res.status(500).json({ error: err.message });
    }
});
server.listen(PORT, () => {
