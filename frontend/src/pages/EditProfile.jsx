...
const EditProfile = () => {
  ...
  const {
    personal_info: { fullname, username, profile_img, email, bio },
    social_links
  } = profile;
          ...
          <div className="flex flex-col lg:flex-row items-start py-10 gap-8 lg:gap-10">
            ...
            <div className="w-full">
              ...
              <div className="md:grid md:grid-cols-2 gap-x-6">
                {Object.entries(social_links).map(([key, link], i) => (
                  <InputBox
                    key={i}
                    name={key}
                    type="text"
                    value={link}
                    placeholder="https://"
                    icon={key !== "website" ? `fi fi-brands-${key}` : "fi fi-rr-globe"}
                  />
                ))}
              </div>

              <button className="btn-dark w-auto px-10 mt-6" type="submit" onClick={handleSubmit}>
                Update
              </button>
            </div>
          </div>
